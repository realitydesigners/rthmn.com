...
interface CourseProgressState {
    ...
    uncompleteLesson: (courseId: string, chapterId: string, lessonId: string) => void;
    isLessonCompleted: (courseId: string, chapterId: string, lessonId: string) => boolean;
    isChapterCompleted: (courseId: string, chapterId: string) => boolean;
    isCourseCompleted: (courseId: string) => boolean;
    getProgress: (courseId: string) => number;
    initializeCourse: (courseId: string, chapters: { _id: string; lessons: { _id: string }[] }[]) => void;
}
    ...
    persist(
        (set, get) => ({
            ...
            isLessonCompleted: (courseId, chapterId, lessonId) => {
                const state = get();
                return state.courses[courseId]?.chapters[chapterId]?.lessons[lessonId]?.completed || false;
            },
