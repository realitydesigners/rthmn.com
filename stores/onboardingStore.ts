...
interface OnboardingState {
	...
	completeStep: (
		stepId: string,
		data?: Partial<OnboardingState["userData"]>,
	) => void;
	goToNextStep: () => void;
	...
	persist(
		(set, get) => ({
			...
			goToNextStep: () => {
				...
				if (currentStep?.type === "page" || !get().hasCompletedPageSteps()) {
					...
					nextIncompleteStep = ONBOARDING_STEPS.filter(
						(step) =>
							step.type === "page" && !state.completedSteps.includes(step.id),
			...
			hasCompletedPageSteps: () => {
				const state = get();
				const pageSteps = ONBOARDING_STEPS.filter(
					(step) => step.type === "page",
				);
				return pageSteps.every((step) =>
					state.completedSteps.includes(step.id),
				);
			},
			...
			getNextIncompleteStep: () => {
				...
				if (currentStep?.type === "page" || !get().hasCompletedPageSteps()) {
					...
					const nextPageStep = pageSteps.find(
