...
const OnboardingCard = ({ step, isCompleted, isCurrent }: { step: (typeof ONBOARDING_STEPS)[0]; isCompleted: boolean; isCurrent: boolean }) => (
    <div
        className={cn(
            'relative w-full overflow-hidden rounded-xl border bg-gradient-to-b p-0.5 transition-all duration-300 hover:scale-[1.02]',
            isCompleted
        ...
        {/* Content */}
        <div className='relative flex flex-col items-center justify-between rounded-xl bg-black/40 p-4'>
            ...
            <div className='ml-4 shrink-0'>
                {isCompleted ? (
                    ...
                    <span className='flex items-center gap-1.5 rounded-full border border-blue-500/20 bg-blue-500/10 px-3 py-1 text-xs font-medium text-blue-400'>In Progress</span>
                ) : (
                    <span className='flex items-center gap-1.5 rounded-full border border-[#333] bg-[#111] px-3 py-1 text-xs font-medium text-gray-500'>Not Started</span>
                )}
            </div>
        </div>
    </div>
        ...
        <div className='space-y-6 p-4'>
            ...
            <div className='mb-6 flex flex-col'>
                ...
                <button
                    ...
                    className='group relative mt-4 rounded-lg bg-gradient-to-b from-blue-500 to-blue-600 px-3 py-1.5 text-xs font-medium text-white transition-all duration-200 hover:from-blue-600 hover:to-blue-700 hover:shadow-lg hover:shadow-blue-500/20'>
            ...
            <div className='space-y-3'>
                {ONBOARDING_STEPS.map((step) => (
                    <OnboardingCard key={step.id} step={step} isCompleted={completedSteps.includes(step.id)} isCurrent={currentStepId === step.id} />
                ))}
            </div>
        </div>
    );
