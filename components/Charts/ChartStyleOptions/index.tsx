			...
			<span
				className={cn(
					...
					locked
						...
						: isActive
							? "text-[#999]"
							: "text-[#666] group-hover:text-[#818181]",
				)}
			>
				{title}
			</span>
...
export const ChartStyleOptions: React.FC<ChartStyleOptionsContainerProps> = ({
	...
	if (noContainer) {
		return (
			<div className={cn("grid grid-cols-3 gap-2", className)}>{content}</div>
		);
	}
	...
	return (
		<div
			className={cn(
				"flex gap-4 rounded-lg border border-neutral-700/50 bg-neutral-800/40 p-4",
				className,
			)}
		>
			{content}
