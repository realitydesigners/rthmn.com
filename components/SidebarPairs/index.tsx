...
interface PairsSidebarProps {
    ...
    currentPair: string;
    getTrendForOffset: (boxes: Box[] | BoxSlice[], offset: number) => 'up' | 'down';
    width: number;
...
const PairsSidebar: React.FC<PairsSidebarProps> = ({ pairs, currentPair, getTrendForOffset, width, onWidthChange }) => {
    ...
    const renderTrendIcon = (trend: 'up' | 'down') => {
        ...
        return (
            <svg className={`h-4 w-4 ${color}`} viewBox='0 0 24 24' fill='none' stroke='currentColor' strokeWidth='2' strokeLinecap='round' strokeLinejoin='round'>
                {trend === 'up' ? <polyline points='23 6 13.5 15.5 8.5 10.5 1 18' /> : <polyline points='23 18 13.5 8.5 8.5 13.5 1 6' />}
    ...
    const handleMouseMove = useCallback(
        (e: MouseEvent) => {
            ...
            const newWidth = Math.min(Math.max(width + deltaX, MIN_SIDEBAR_WIDTH), MAX_SIDEBAR_WIDTH);
    ...
    return (
        <div
            ...
            <DraggableBorder isDragging={isDragging} onDragStart={handleDragStart} direction='left' />
            ...
            <ul className='space-y-1'>
                {Object.entries(pairs).map(([pair, data]) => (
                    <li
                        key={pair}
                        className={`flex flex-col text-sm ${pair === currentPair ? 'bg-[#000]' : ''} ${selectedPair === pair ? 'overflow-hidden border border-[#222]' : ''}`}>
                        <div className={`flex items-center justify-between py-2 hover:bg-[#111] ${selectedPair === pair ? 'border-b border-[#222]' : ''}`}>
                            <button onClick={() => handlePairClick(pair)} className='flex w-full items-center'>
                                <span className='w-24 pl-1 font-bold'>{pair}</span>
                                <span className='w-24 pl-1 text-[10px] text-gray-300'>{formatValue(data.currentOHLC.close)}</span>
                            </button>
                            <div className='flex space-x-1 pr-1'>
                                {offsets.map(({ label, value }) => {
                                    ...
                                    return (
                                        <button
                                            ...
                                            {hoveredOffset === `${pair}-${label}` && <span className='ml-1 text-[12px]'>{label}</span>}
                                        </button>
                                    );
                                })}
                            </div>
...
export const getTrendForOffset = (boxes: Box[] | BoxSlice[], offset: number): 'up' | 'down' => {
    if (!boxes || boxes.length === 0) {
        return 'down';
    }

    const allBoxes: Box[] = 'boxes' in boxes[0] ? (boxes as BoxSlice[]).flatMap((slice) => slice.boxes) : (boxes as Box[]);
    ...
    const largestBox = boxesForOffset.reduce((max, box) => (Math.abs(box.value) > Math.abs(max.value) ? box : max));
