...
export function MarketDisplay({ marketData }: SectionMarketDisplayProps) {
    ...
    const sortedPairs = [...marketData].sort((a, b) => a.pair.localeCompare(b.pair));
                    ...
                    return (
                        <motion.div
                            ...
                            <div className='mb-2 flex items-start justify-between'>
                                ...
                                <span className={`font-kodemono text-xs ${priceChange && priceChange >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                                    {priceChange ? `${priceChange.toFixed(2)}%` : 'N/A'}
                                </span>
                            </div>
                            <div className='font-kodemono mb-2 text-2xl font-bold text-white'>{latestPrice ? latestPrice.toFixed(item.pair.includes('JPY') ? 3 : 5) : 'N/A'}</div>
                            {getDayHighLow(item.candleData) && (
                                <div className='font-kodemono mb-6 flex justify-between text-xs text-white/60'>
                                    <span>High: {getDayHighLow(item.candleData)?.high.toFixed(item.pair.includes('JPY') ? 3 : 5)}</span>
                                    <span>Low: {getDayHighLow(item.candleData)?.low.toFixed(item.pair.includes('JPY') ? 3 : 5)}</span>
                                </div>
                            )}
                            <div className='mb-2 h-16 w-full'>
                                {getSparklinePoints(item.candleData, 200, 60) && (
                                    <svg width='100%' height='100%' viewBox='0 0 200 60' preserveAspectRatio='none' className='overflow-visible'>
                                        <polyline
                                            points={getSparklinePoints(item.candleData, 200, 60) || ''}
                                            fill='none'
                                            stroke={getPriceChange(item.candleData) >= 0 ? '#4ade80' : '#f87171'}
                                            strokeWidth='1.5'
