...
export default function SupportPanel({ isOpen, onClose }: SupportPanelProps) {
    ...
    useEffect(() => {
        const fetchThreads = async () => {
            try {
                ...
                setError(error instanceof Error ? error.message : 'Failed to load support threads');
            }
        };
    ...
    const handleCreateThread = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setError(error instanceof Error ? error.message : 'Failed to create support thread');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleSendMessage = async (e: React.FormEvent) => {
        ...
        clearError();
        try {
            const data = await sendSupportMessage(supabase, selectedThread.id, newMessage);
            if (data) {
                ...
                <div className='scrollbar-thin scrollbar-track-transparent scrollbar-thumb-white/10 flex-1 overflow-y-auto'>
                    {!selectedThread && !isCreatingThread ? (
                                ...
                                {threads.map((thread) => (
                                    <div
                                        ...
                                        <div className='mt-2 flex items-center gap-2'>
                                            {thread.status === 'open' ? (
                                                <span className='font-outfit text-xs text-green-500'>Open</span>
                                            ) : (
                        ...
                        <form onSubmit={handleCreateThread} className='p-4'>
                            <div>
                                <label className='font-outfit block text-sm font-medium text-zinc-400'>Subject</label>
                                <input
                                    type='text'
