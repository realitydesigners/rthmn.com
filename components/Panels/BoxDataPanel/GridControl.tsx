...
const GridIcon = React.memo(
	({
		...
		isActive,
		onClick,
	}: { cols: number; isActive: boolean; onClick: () => void }) => {
		...
		return (
			<button
				...
				className={`group relative flex h-7 w-7 items-center justify-center rounded-md transition-all duration-200 ${
					isActive
						? "border-[#1C1E23]  bg-gradient-to-b from-[#1A1A1A] to-[#141414]"
						: "border-[#0A0B0D] bg-gradient-to-b from-[#111] to-[#0A0A0A] hover:border-[#1C1E23]  hover:from-[#151515] hover:to-[#0F0F0F]"
				}`}
			>
					...
					{cells.map((i) => (
						<div
							...
							className={`rounded-[1px] border transition-all duration-200 ${
								isActive
									? "border-[#32353C] bg-[#0A0B0D]"
									: "border-[#0A0B0D] bg-[#1C1E23] group-hover:border-[#1C1E23]  group-hover:bg-[#1A1A1A]"
							}`}
						/>
					))}
				</div>
