...
import { ScaledIcon, EvenIcon, OscillatorIcon, MinusIcon, PlusIcon } from '@/components/Icons/icons';
...
const HistogramControls: React.FC<HistogramControlsProps> = ({ boxOffset, onOffsetChange, totalBoxes, visibleBoxesCount, viewType, onViewChange }) => {
    const cycleViewType = () => {
    ...
    return (
        <div className='absolute top-0 right-0 flex h-full w-16 flex-col items-center justify-center border-l border-[#181818] bg-black'>
            <AdjusterButton icon={<PlusIcon />} onClick={() => onOffsetChange(Math.max(0, boxOffset - 1))} disabled={boxOffset === 0} />
            ...
            />
            <button onClick={cycleViewType} className='mt-2 flex h-10 w-10 items-center justify-center rounded-sm border border-[#181818] bg-black text-white hover:bg-[#181818]'>
