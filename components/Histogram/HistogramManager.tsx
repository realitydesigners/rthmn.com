...
interface HistogramManagerProps {
  ...
  onVisibleBoxesCountChange: (newCount: number) => void;
}
...
const HistogramManager: React.FC<HistogramManagerProps> = ({
  ...
  onOffsetChange,
  visibleBoxesCount,
  onVisibleBoxesCountChange
}) => {
  const [viewType, setViewType] = useState<'scaled' | 'even' | 'oscillator'>(
    'oscillator'
  );
  ...
  const handleViewChange = useCallback(
    (newViewType: 'scaled' | 'even' | 'oscillator') => {
  ...
  const handleRangeChange = useCallback(
    (start: number, end: number) => {
      ...
      const newVisibleBoxesCount = end - start + 1;
      onVisibleBoxesCountChange(newVisibleBoxesCount);
    },
    [onOffsetChange, onVisibleBoxesCountChange]
  );
