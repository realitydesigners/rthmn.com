...
const BoxCourseVisualizer = ({ value }: BoxVisualizerProps) => {
	...
	const currentSlice = useMemo(() => {
		...
		const sortedBoxes = mockBoxData.sort(
			(a, b) => Math.abs(b.value) - Math.abs(a.value),
		);
		return {
	...
	useEffect(() => {
		...
		const interval = setInterval(() => {
			...
			const shouldPauseAtChange =
				value.pointOfChangeIndex !== undefined && value.pauseDuration !== 0;

			if (
				shouldPauseAtChange &&
				currentPatternIndex === activeSequences.length - 1 &&
				!isPaused
			) {
				setIsPaused(true);
				setTimeout(() => {
