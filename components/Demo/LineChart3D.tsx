...
const RectangleSections = ({ width, sections }: { 
  width: number; 
...
const FluidLine = ({ time, onBoundaryHit, onProgressUpdate }: { 
  time: number; 
  ...
  const geometry = useMemo(() => {
    ...
    while (pointsHistoryRef.current.length < targetPoints && pointsHistoryRef.current.length < 1000) {
    ...
    const visiblePoints = pointsHistoryRef.current.slice(trimPointCount, targetPoints);
...
const AnimationController = () => {
  ...
  const [boundaryHits, setBoundaryHits] = useState<Array<{x: number, isTop: boolean}>>([]);
  ...
  useFrame((state) => {
    ...
    const targetPoints = Math.floor((state.clock.elapsedTime + timeOffset) / timeStep);
  ...
  return (
    <>
      ...
      <FluidLine time={time} onBoundaryHit={handleBoundaryHit} onProgressUpdate={handleProgressUpdate} />
    </>
  );
};
