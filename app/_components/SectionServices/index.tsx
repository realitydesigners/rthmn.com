...
export const ServiceSection = () => {
	...
	return (
		<div className="flex flex-row flex-wrap px-4 md:px-40 py-10 text-white w-full">
			<div
				className={`text-balance transition-all duration-300 ease-in-out ${
					isDetailVisible
			...
			>
				...
				>
					{Services.map(({ label, icon }) => (
						<li
							key={label}
							className={`p-4 border  heading-text w-full border-gray-600/50 cursor-pointer ${
								activeService === label
									? "bg-gray-600/25"
						...
						>
							{getIcon(icon)}
							<div
								className={`uppercase font-bold leading-[1.2em] text-lg ml-2 ${oxanium.className}`}
							>
								{label}
							</div>
						</li>
			...
			{isDetailVisible && (
				...
				>
					<h2
						className={`leading-none heading-text font-bold uppercase text-4xl lg:text-5xl mb-4 fade-in ${oxanium.className}`}
					>
						{activeService}
					</h2>
					<p
						className={`text-xl primary-text text-gray-400 fade-in  ${oxanium.className}`}
					>
