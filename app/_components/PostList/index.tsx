...
const PostItem: React.FC<{ post: Post; index: number }> = ({ post, index }) => {
  ...
  return (
    <motion.div
      className="group flex h-full flex-col overflow-hidden rounded-lg border border-[#181818] shadow-lg transition-all duration-300 hover:scale-105"
    ...
    >
      ...
      <div className="flex flex-grow flex-col p-4">
        ...
        <p
          className={`${oxanium.className} mb-4 line-clamp-3 flex-grow text-sm text-gray-400`}
        >
          {block?.subheading || 'No subheading'}
        </p>
        <Link
          href={`/posts/${post.slug.current}`}
          className={` ${outfit.className} self-start rounded-full bg-gradient-to-b from-[#333333] to-[#181818] px-4 py-2 pb-3 text-sm font-semibold text-white transition-all duration-200 hover:from-[#444444] hover:to-[#282828]`}
        >
          Read More
        </Link>
      </div>
...
export function PostList({ initialPosts }: PostListProps) {
  ...
  return (
    <div className={`mt-16 flex w-full flex-col ${oxanium.className}`}>
      <h2
