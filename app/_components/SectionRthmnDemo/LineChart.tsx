...
export const LineChart: React.FC<{
    ...
    const scaledData = useMemo(() => {
        ...
        return visibleData.map((point, index) => {
            const scaledX = ensureNumber(index * (chartWidth / (visibleData.length - 1)));
            const scaledY = ensureNumber(chartHeight / 2 - ((point.price - midPrice) * yScale) / animatedScale);
            return {
                ...point,
                scaledX,
                scaledY,
...
const ChartLine: React.FC<{
    ...
    return <path ref={pathRef} stroke='white' strokeWidth='1.5' fill='none' className='bg-white' />;
});
...
}> = React.memo(({ minY, maxY, chartHeight, chartWidth, onDrag, onScale, yAxisScale, hoverInfo }) => {
