...
export const BoxSection: React.FC<AutoBoxModuleProps> = ({ splineRef, visibility }) => {
    ...
    // Helper functions
    const calculateBoxDimensions = (index: number, baseSize: number, scaleFactor: number): any => {
    ...
    const calculateCornerOffset = (currentBox: any, parentBox: any, scaleFactor: number): number => {
        ...
        try {
            ...
            for (let i = 1; i <= 7; i++) {
                ...
                if (box && parentBox) {
                    ...
                    const finalY = currentParentY + (config[i] === 1 ? cornerOffset : -cornerOffset);
                        ...
                        {boxStates.map((box, index) => (
                            <div key={box.name} className='group relative'>
                                <div
                                    ...
                                    <span className={`font-mono text-[10px] font-bold ${box.position === 1 ? 'text-blue-400' : 'text-blue-300/50'}`}>
                                        {box.position === 1 ? '1' : '0'}
                                    </span>
                                </div>
                            </div>
