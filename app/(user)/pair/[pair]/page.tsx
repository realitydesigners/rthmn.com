...
async function fetchApiData(
  ...
  try {
    ...
    const endpoint = hasSubscription
      ? `/candles/${pair.toUpperCase()}?limit=${CANDLE_LIMIT}&interval=1min`
      : `/public/candles/${pair.toUpperCase()}?limit=${CANDLE_LIMIT}&interval=1min`;
    ...
    console.log("🎉 Processed Data Summary:", {
      ...
      firstTimestamp: new Date(
        processedData[0]?.timestamp * 1000
      ).toISOString(),
      lastTimestamp: new Date(
        processedData[processedData.length - 1]?.timestamp * 1000
      ).toISOString(),
      sampleCandle: processedData[0],
    });
