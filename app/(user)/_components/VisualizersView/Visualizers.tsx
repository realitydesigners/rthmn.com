...
const TIMEFRAMES = ['D', '12H', '4H', '2H', '1H', '15m', '5m', '1m'] as const;
const SEGMENT_WIDTH = 38 / 9;
...
export const TimeFrameVisualizer: React.FC<PatternVisualizerProps> = ({ startIndex, maxBoxCount, boxes, onStyleChange }) => {
                        ...
                        <div className='absolute inset-x-0 bottom-2 flex justify-between px-[10px]'>
                            <div className='font-kodemono font- relative flex w-full justify-between text-[8px] uppercase'>
                                {TIMEFRAMES.map((time, i) => {
                                    ...
                                    const segmentWidth = SEGMENT_WIDTH; // Width of each timeframe segment
                                    ...
                                    return (
                                        <div key={time} className='relative flex flex-col items-center'>
                                            ...
                                            />
                                            <span className={cn('transition-all duration-300', isInRange ? 'font-medium text-white' : 'text-white/30')}>{time.toLowerCase()}</span>
                                        </div>
                                    );
                                })}
                            </div>
