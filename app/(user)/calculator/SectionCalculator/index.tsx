...
const validateInputs = (inputs: CalculatorInputs): ValidationError[] => {
    ...
    if (isNaN(inputs.riskPercentage) || inputs.riskPercentage <= 0 || inputs.riskPercentage > 100) {
        errors.push({
    ...
    ({
        ...
        <div className='relative'>
            <label className='font-kodemono mb-2 flex items-center justify-between text-sm'>
                ...
                {error && <span className='text-xs text-red-400 transition-all duration-300'>{error}</span>}
            </label>
            <div className='relative'>
...
export const SectionCalculator = () => {
    ...
    const results = useMemo(() => {
        ...
        const potentialProfit = positionSize * Math.abs(inputs.takeProfit - inputs.entryPrice);
    ...
    const formatValue = useCallback(
        (value: number) => {
            if (isNaN(value) || !isFinite(value)) return `${getCurrencySymbol(inputs.currency)}0.00`;
            return `${getCurrencySymbol(inputs.currency)}${value.toLocaleString('en-US', {
                ...
                <div className='mx-auto flex gap-8'>
                    ...
                    <div className='hidden w-1/4 shrink-0 lg:block'>
                        <div className='sticky top-8 rounded-xl border border-white/10 bg-black/40 p-6'>
                            <h3 className='font-outfit mb-6 text-lg font-semibold text-white'>Quick Risk Templates</h3>
                    ...
                    <div className='w-full lg:w-1/2'>
                        ...
                        <div className='mb-8 rounded-xl border border-white/10 bg-black/40 p-6'>
                            <h3 className='font-outfit mb-6 text-lg font-semibold text-white'>Account Details</h3>
                            <div className='grid gap-6 md:grid-cols-3'>
                                ...
                                />
                                <CurrencySelector currency={inputs.currency} onCurrencyChange={handleCurrencyChange} />
                                <InputField
                        ...
                        <div className='mb-8 rounded-xl border border-white/10 bg-black/40 p-6'>
                            <h3 className='font-outfit mb-6 text-lg font-semibold text-white'>Trade Direction</h3>
                            <TradeDirection direction={inputs.direction} onDirectionChange={handleDirectionChange} />
                        </div>
                    ...
                    <div className='hidden w-1/4 space-y-4 lg:block'>
                        <div className='sticky top-8 space-y-4'>
                            ...
                            <ResultCard
                                label='Trade Summary'
                                value={`${formatValue(results.riskAmount)} → ${formatValue(results.potentialProfit)}`}
                                subValue={`Risk ${inputs.riskPercentage}% for ${((results.potentialProfit / results.riskAmount - 1) * 100).toFixed(1)}% return`}
                                icon={<FaChartLine className='h-5 w-5' />}
                                positive={results.potentialProfit > results.riskAmount}
                                isInvalid={isNaN(results.potentialProfit) || isNaN(results.riskAmount)}
                            />
                        </div>
                    </div>
                </div>
