...
const Dashboard = ({ boxStates, configs, isAnimating, onConfigSelect }: DashboardProps) => {
    return (
...
export default function App() {
    ...
    const [activePanel, setActivePanel] = useState<'status' | 'config' | null>(null);
    ...
    const processBox = async ({ index, config }: ProcessBoxParams) => {
        ...
        const parentBox = splineRef.current.findObjectByName(GREEN_BOXES[index - 1]);
        ...
        const targetY = parentBox.position.y + (config[index] === 'Up' ? cornerOffset : -cornerOffset);
            ...
            boxNames.forEach((name, index) => {
                ...
                if (currentBox) {
                    ...
                    try {
                        ...
                        const currentDimensions = calculateBoxDimensions(index, baseSize, scaleFactor);
                        ...
                        if (index > 0) {
                            ...
                            if (parentBox) {
                                ...
                                const parentDimensions = calculateBoxDimensions(index - 1, baseSize, scaleFactor);

                                // Calculate corner offset using sqrt(2) relationship
                                const cornerOffset = calculateCornerOffset(currentDimensions, parentDimensions, scaleFactor);
    ...
    return (
        <main className='relative flex h-full w-full overflow-hidden'>
            ...
            <Dashboard boxStates={boxStates} configs={configs} isAnimating={isAnimating} onConfigSelect={applyBoxConfiguration} />
        </main>
    );
}
...
const SplineScene = ({ onLoad }: SplineSceneProps) => {
    return <Spline scene='https://prod.spline.design/ojzfkQJP2K4w4LC6/scene.splinecode' onLoad={onLoad} />;
};
