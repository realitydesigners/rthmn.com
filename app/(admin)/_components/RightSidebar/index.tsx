...
const BoxValuesTable = ({ pair }: { pair: string }) => {
    ...
    if (!boxHistory.length) {
        return <div className='text-neutral-400'>No box data available...</div>;
    }
                ...
                <tbody>
                    {latestBoxes.map((box, index) => (
                        <tr key={index} className='hover:bg-black/20'>
                            <td className='p-1 text-neutral-500'>{index + 1}</td>
    ...
    return (
        <div className='rounded border border-[#181818] bg-[#0a0a0a]'>
            <button onClick={() => setIsOpen(!isOpen)} className='flex w-full items-center justify-between p-2 hover:bg-[#111111]'>
                <h4 className='text-[11px] font-medium text-neutral-300'>
...
export function RightSidebar({
    ...
    return (
        <div className='fixed right-0 top-0 mt-14 hidden h-screen w-[400px] overflow-y-auto border-l border-[#181818] bg-black p-4 lg:block'>
                    ...
                    <div className='space-y-2'>
                            ...
                            <div className='flex items-center gap-2'>
                                <div className='flex items-center gap-2 text-[11px]'>
                                    ...
                                    <input
                                        ...
                                        className='w-16 rounded border border-[#181818] bg-black px-1 py-0.5 text-neutral-300'
                                    />
                                </div>
                                <button onClick={handleFetchClick} className='border border-[#181818] bg-black px-2 py-1 text-[11px] text-neutral-300 hover:bg-[#111111]'>
                                    Fetch
                                </button>
                            </div>
                            <div className='flex items-center gap-2'>
                                <button
                                    ...
                                    className={`border border-[#181818] px-2 py-1 text-[11px] text-neutral-300 transition-colors ${
                                ...
                                <label className='flex items-center gap-1 text-[11px] text-neutral-400'>
                                    <input
                                        ...
                                        onChange={(e) => setIsChronological(e.target.checked)}
                        ...
                        <div className='space-y-0.5'>
                            {isLoading ? (
                                ...
                                showJson ? (
                                    ...
                                    candles.map((candle, index) => (
                                        <div key={index} className='flex justify-between font-mono text-[11px]'>
                                            <span className='text-neutral-500'>{formatTime(candle.timestamp)}</span>
                                ...
                                <div className='text-[11px] text-neutral-500'>No data available</div>
                            )}
                        </div>
                    </div>
                </CollapsibleSection>
                ...
                <CollapsibleSection title='Box Slices' count={pairBoxSlices?.length}>
                    <div>
                        <button
                            ...
                            className='mb-2 w-full border border-[#181818] bg-black px-2 py-2 text-[11px] text-neutral-300 transition-colors hover:bg-[#181818]'>
                            Refresh Box Slices
                        </button>
                        <div className='overflow-auto'>
                            {isLoading ? (
                                ...
                                <div className='text-[11px] text-neutral-500'>No box slices available</div>
                            )}
                        </div>
                    </div>
                </CollapsibleSection>
