...
export async function GET(request: Request) {
    ...
    try {
        ...
        const { data: subscription } = await supabase.from('subscriptions').select('*').in('status', ['trialing', 'active']).single();
        ...
        return NextResponse.redirect(`${origin}/account?error=discord_connection_failed`);
    }
}
