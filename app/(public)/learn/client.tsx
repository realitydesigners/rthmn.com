    ...
    return (
        <div className='relative min-h-screen w-full overflow-hidden bg-black'>
            ...
            <div className='relative'>
                {/* Hero Section */}
                <div className='mx-auto mt-40 max-w-5xl px-6 text-center sm:px-8 lg:px-12'>
                            ...
                            <div className='overflow-hidden rounded-lg border border-white/10 bg-white/5 p-1'>
                                <div className='flex items-center gap-2 px-3 py-1'>
                                    ...
                                    <span className='font-kodemono text-sm text-gray-400'>
                                        {courses.reduce((total, course) => total + course.modules.length, 0)} modules that take you from zero to hero
                                    </span>
                                </div>
                            </div>
                        </div>
                ...
                <div className='mx-auto max-w-5xl px-6 pb-24 sm:px-8 lg:px-12'>
                    ...
                    <div className='mb-16 space-y-6'>
                        <h2 className='font-outfit text-2xl font-semibold text-white'>Learning Tracks</h2>
                    ...
                    {/* Course Modules */}
                    {courses.map((course) => (
                        <div key={course._id} className='space-y-12'>
                            <h2 className='font-outfit text-2xl font-semibold text-white'>{course.title} Modules</h2>
                            <div className='space-y-6'>
                                {course.modules?.map((module, moduleIndex) => {
                                    const status = getModuleStatus(course._id, module._id);
                                    const isLocked = moduleIndex > 0 && getModuleStatus(course._id, course.modules[moduleIndex - 1]._id) !== 'completed';

                                    return (
                                        <div
                                            key={module._id}
                                            className='rounded-xl border border-white/10 bg-white/5 p-6 transition-all duration-300 hover:border-emerald-500/20 hover:bg-emerald-500/5'>
                                            <div className='mb-6 flex items-center justify-between'>
                                                ...
                                                {status === 'completed' ? (
                                                    ...
                                                    <button
                                                        onClick={() => {
                                                            if (status === 'not-started') startCourse(course._id);
                                                            startModule(course._id, module._id);
                                                        }}
                                                        className='rounded-full bg-emerald-400/10 px-4 py-2 text-sm text-emerald-400 hover:bg-emerald-400/20'>
                                                        {status === 'not-started' ? 'Start Module' : 'Continue'}
                                                    </button>
                                                )}
                                            </div>
                                            ...
                                            <div className='space-y-2'>
                                                {module.lessons?.map((lesson, lessonIndex) => {
                                                    const lessonStatus = getLessonStatus(course._id, module._id, lesson._id);
                                                    return (
                                                        <MotionLink
                                                            key={lesson._id}
                                                            href={lessonStatus === 'locked' ? '#' : `/learn/${course.slug.current}/${module.slug.current}/${lesson.slug.current}`}
                                                            className={`flex items-center justify-between rounded-lg border p-4 transition-all duration-300 ${
                                                                lessonStatus === 'locked'
                                                                    ...
                                                                    : lessonStatus === 'completed'
                                                                      ...
                                                                      : 'border-white/5 bg-white/5 hover:border-emerald-500/30 hover:bg-emerald-500/5'
                                                            }`}
                                                            whileHover={lessonStatus !== 'locked' ? { scale: 1.01 } : {}}
                                                            whileTap={lessonStatus !== 'locked' ? { scale: 0.99 } : {}}>
                                                            <div className='flex items-center gap-3'>
                                                                <div className='flex h-6 w-6 items-center justify-center rounded-full bg-white/5 text-sm text-gray-400'>
                                                                    {lessonIndex + 1}
                                                                </div>
                                                                <div>
                                                            ...
                                                            {lessonStatus === 'completed' ? (
                                                                ...
                                                                <FaPlay className='h-5 w-5 text-emerald-400' />
                                                            )}
                                                        </MotionLink>
                                                    );
                                                })}
