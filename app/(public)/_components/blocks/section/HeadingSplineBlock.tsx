    ...
    switch (className) {
        case 'dark': {
            ...
            const renderCategory = block.category ? (
                <span
                    className={`font-russo mr-1 h-auto items-center justify-center bg-gray-200 p-1 pr-2 pl-2 text-xs font-semibold tracking-widest whitespace-nowrap text-black uppercase`}>
                    ...
                    <div className='flex w-full flex-wrap justify-center'>
                        <div className='flex-cols flex w-11/12 flex-wrap items-center justify-between'>
                            <div className='flex w-auto'>
                                <span className={`font-russo ml-2 w-auto text-xs tracking-widest text-gray-200 uppercase`}>POSTED ON {formattedDate}</span>
                            </div>
                            {renderCategory}
                        </div>

                        <div className='w-full flex-col lg:w-1/2'>
                            {block.heading && <h1 className={`leading-tightest font-russo p-4 text-5xl text-gray-200 md:text-7xl`}>{block.heading}</h1>}
                            {block.subheading && <h2 className={`w-full p-4 text-2xl leading-7 tracking-wide text-gray-300`}>{block.subheading}</h2>}
                        </div>
                                        ...
                                        <div className='flex w-auto p-2'>
                                            {block.team?.image && (
                                                <div className='flex items-center'>
                                                    ...
                                                    />
                                                    {block.team.name && <span className='ml-2 text-sm tracking-wide text-gray-200 uppercase'>{block.team.name}</span>}
                                                </div>
                                            )}
                                        </div>
                                    </Link>
