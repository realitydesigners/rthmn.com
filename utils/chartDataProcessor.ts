...
export function processLiveCandleUpdate(currentCandles: ChartDataPoint[], update: LiveCandleUpdate, currentCandleRef: { current: ChartDataPoint | null }): ChartDataPoint[] {
    ...
    if (!currentCandleRef.current) {
        ...
        const minutesDiff = Math.floor((currentMinute - lastHistoricalCandle.timestamp) / (60 * 1000));
        ...
        if (minutesDiff <= 1) {
            ...
            lastHistoricalCandle.high = Math.max(lastHistoricalCandle.high, update.price);
            lastHistoricalCandle.low = Math.min(lastHistoricalCandle.low, update.price);
    ...
    if (currentCandleRef.current.timestamp !== currentMinute) {
        ...
        const minutesDiff = Math.floor((currentMinute - lastCandleTime) / (60 * 1000));
