...
export async function manageDiscordAccess(stripeCustomerId: string, isActive: boolean) {
    ...
    try {
        ...
        if (isActive) {
            ...
            await Promise.all([
                member.roles.add(process.env.DISCORD_PAID_ROLE_ID!).then(() => console.log('Added paid role')),
                member.roles.remove(process.env.DISCORD_UNPAID_ROLE_ID!).then(() => console.log('Removed unpaid role')),
            ]);
        } else {
            // Remove paid role and add unpaid role
            await Promise.all([
                member.roles.remove(process.env.DISCORD_PAID_ROLE_ID!).then(() => console.log('Removed paid role')),
                member.roles.add(process.env.DISCORD_UNPAID_ROLE_ID!).then(() => console.log('Added unpaid role')),
            ]);
        }
