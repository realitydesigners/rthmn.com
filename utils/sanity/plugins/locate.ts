...
export const locate: DocumentLocationResolver = (params, context) => {
    ...
    if (params.type === 'home' || params.type === 'page' || params.type === 'project') {
        const doc$ = context.documentStore.listenQuery(`*[_id==$id || references($id)]{_type,slug,title}`, params, { perspective: 'previewDrafts' }) as Observable<
            | {
                  _type: string;
                  slug: { current: string };
                  title: string | null;
                ...
                switch (params.type) {
                    ...
                    case 'project':
                        return {
                            ...
                            message: isReferencedBySettings ? 'This document is used on all pages as it is in the top menu' : undefined,
                        } satisfies DocumentLocationsState;
                    default:
                        return {
