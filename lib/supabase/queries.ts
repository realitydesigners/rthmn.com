...
export const getProducts = cache(async (supabase: SupabaseClient) => {
    try {
        ...
        // Fetch prices
        const { data: prices, error: pricesError } = await supabase.from('prices').select('*').eq('active', true);
...
export const getUserDetails = cache(async (supabase: SupabaseClient) => {
    try {
        ...
        const { data: userDetails, error } = await supabase.from('users').select('*').eq('id', user.id).single();
...
export const getDiscordConnection = cache(async (supabase: SupabaseClient) => {
    try {
        const { data: discord, error } = await supabase.from('discord_connections').select('*').maybeSingle();
